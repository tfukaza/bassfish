...
Retained-memory checkpoints await complete major collection at an asynchronous event-loop boundary, then yield for native weak finalizers. Node 24.12's [V8 GC implementation](https://github.com/nodejs/node/blob/v24.12.0/deps/v8/src/extensions/gc-extension.cc) conservatively scans stack pointers during synchronous GC; an asynchronous checkpoint uses a clean stack and the `last-resort` flavor completes all available collection. This distinguishes retained allocations from native wrappers waiting for finalization. The footprint and growth thresholds remain unchanged; these test-only checkpoints do not add GC calls to the production daemon. This harness is Node-only: Bun runs JavaScriptCore, where `--expose-gc` performs a synchronous JSC collection and offers no equivalent control, so `npm run test:memory` and `npm run test:memory-soak` qualify the Node runtime alone. The daemon itself runs under Bun; only this measurement does not.
