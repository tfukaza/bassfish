...
test('the MCP boundary presents compact public data and acquires turns in one operation', async t => {
  ...
  const created = await call<{ threadId: string; revision: string }>(
    f.a.agentHandle,
    'createResource',
    {
      resourceType: 'thread',
  ...
  const found = await call<{ resource: { threadId: string; descriptionPreview: string } }>(
    f.a.agentHandle,
    'findResources',
    {
      resourceType: 'thread',
  ...
  const page = await call<{ messages: unknown[] }>(f.a.agentHandle, 'readTurn', {
    view: 'page',
    turnToken: claimed.turnToken,
  });
  assert.deepEqual(page.messages, []);
  const committed = await call<{ threadId: string; revision: string; messageId: string }>(
    f.a.agentHandle,
    'commitTurn',
    {
      turnToken: claimed.turnToken,
  ...
  const ticket = await call<{ ticketId: string }>(f.a.agentHandle, 'createResource', {
    resourceType: 'ticket',
    title: 'Public ticket',
    description: 'Work',
    owner: 'Alice',
  ...
  const ticketTurn = await call<{ turnToken: string; text: string }>(
    f.a.agentHandle,
    'acquireTurn',
    { target: { type: 'ticket', ticketId: ticket.ticketId } },
  );
  assert.equal(ticketTurn.text, '# One\nsecret body');
  const outline = await call<{ headings: { heading: string }[] }>(f.a.agentHandle, 'readTurn', {
    view: 'outline',
    turnToken: ticketTurn.turnToken,
  });
  assert.equal(outline.headings[0]!.heading, 'One');
  const matches = await call<{ matches: unknown[] }>(f.a.agentHandle, 'readTurn', {
    view: 'find',
    turnToken: ticketTurn.turnToken,
    query: 'secret',
  });
  ...
  const holder = await call<{ turnToken: string }>(f.a.agentHandle, 'acquireTurn', {
    target: { type: 'ticket', ticketId: ticket.ticketId },
  });
  const queued = await call<{ state: string; requestToken: string }>(
    f.b.agentHandle,
    'acquireTurn',
    { target: { type: 'ticket', ticketId: ticket.ticketId }, timeoutMs: 0 },
  );
  ...
  const replayed = await call<{ requestToken: string; turnToken: string; text: string }>(
    f.b.agentHandle,
    'acquireTurn',
  ...
  const cancelHolder = await call<{ turnToken: string }>(f.a.agentHandle, 'acquireTurn', {
    target: { type: 'ticket', ticketId: ticket.ticketId },
  });
  const abandoned = await call<{ requestToken: string }>(f.b.agentHandle, 'acquireTurn', {
    target: { type: 'ticket', ticketId: ticket.ticketId },
    timeoutMs: 0,
  });
  const cancelled = await call<{ state: string }>(f.b.agentHandle, 'cancelTurn', {
...
test('a queued MCP Task claims only when polled and persists only a claim reference', async t => {
  ...
  assert.equal(
    f.control.view(state => state.requests[taskId]!.state),
    'READY',
  );

  const completed = presentTask(await f.service.getTask(f.b.agentHandle, taskId)) as {
    status: string;
    result: { state: string; requestToken: string; turnToken: string; messages: unknown[] };
  };
  ...
  assert.deepEqual(
    f.control.view(state => state.tasks[taskId]!.result),
    { requestId: taskId },
  );
