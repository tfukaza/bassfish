...
test('SQLite transactions reject duplicate ownership and roll back all changes', async t => {
  ...
  assert.equal(
    f.control.view(s => Object.values(s.requests).length),
    1,
  );
  assert.equal(
    f.control.view(s => Object.values(s.projects)[0]!.recovering),
    false,
  );
});
...
test('a future SQLite schema is rejected without changing its version', async t => {
  ...
  existing.exec('PRAGMA user_version=999');
  existing.close();
  assert.throws(() => new SqliteControl(path), errorCode('SCHEMA_MISMATCH'));
  const inspect = new DatabaseSync(path, { readOnly: true });
  try {
    assert.equal(inspect.prepare('PRAGMA user_version').get()!.user_version, 999);
  } finally {
    inspect.close();
  }
});

test('control schemas older than v5 still require an explicit reset', async t => {
  const dir = await mkdtemp(join(tmpdir(), 'bf-schema-v3-'));
  t.after(() => rm(dir, { recursive: true, force: true }));
  const path = join(dir, 'control.sqlite'),
    existing = new DatabaseSync(path);
  existing.exec('PRAGMA user_version=3');
  existing.close();
  assert.throws(() => new SqliteControl(path), errorCode('SCHEMA_MISMATCH'));
  const inspect = new DatabaseSync(path, { readOnly: true });
  try {
