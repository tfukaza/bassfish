...
export class Bassfish {
  ...
  async initialize(): Promise<void> {
    ...
    this.control.update(state => {
      ...
      for (const request of values(state.requests)) {
        ...
        if (request.state === 'OFFERED' || request.state === 'HELD') this.finish(request, 'EXPIRED', state);
      }
  ...
  private creationAllowed(state: ControlState, projectId: string): void {
    ...
    requireThat(!values(state.requests).some(request => request.projectId === projectId && request.resourceType === 'project' && activeStates.includes(request.state)),
      'PROJECT_FLOOR_PENDING', 'A project floor is queued or active; create the resource after it finishes.');
  }
  private finish(request: FloorRequest, state: 'EXPIRED' | 'CANCELLED' | 'RELEASED' | 'FAILED' | 'COMMITTED', control?: ControlState): void {
    request.state = state;
    ...
    if (task) {
      ...
      if (state === 'FAILED') { task.status = 'failed'; task.statusMessage = 'The floor request failed.'; task.error = { code: -32603, message: 'The floor request failed.' }; }
      else if (state !== 'COMMITTED') { task.status = 'cancelled'; task.statusMessage = state === 'EXPIRED' ? 'The floor request expired.' : 'The floor request was cancelled.'; }
    }
  }
  private disconnectIn(state: ControlState, instance: Instance, clean: boolean): void {
    ...
    for (const request of values(state.requests).filter(r => r.instanceId === instance.id)) {
      ...
      if (request.state === 'HELD' || request.state === 'OFFERED') this.finish(request, clean ? 'RELEASED' : 'EXPIRED', state);
    }
  }
  ...
  sweep(): void {
    ...
    this.control.update(state => {
      ...
      for (const request of values(state.requests)) {
        ...
        if (request.state === 'HELD' && (discontinuity || now >= request.expiresAt!)) this.finish(request, 'EXPIRED', state);
  ...
  private promote(state: ControlState): void {
    for (const project of values(state.projects)) {
      ...
      if (barrier) {
        ...
        if (all.some(r => r.resourceType !== 'project' && (r.state === 'HELD' || r.state === 'COMMITTING'))) continue;
  ...
  private makeAvailable(state: ControlState, request: FloorRequest): void {
    ...
    if (request.deliveryMode === 'task') {
      ...
      if (task) { task.updatedAt = request.updatedAt; task.statusMessage = 'Floor ready; poll the task to receive a 30-second offer.'; }
      return;
    }
    request.state = 'OFFERED'; request.offerId = uid(); request.claimBy = request.updatedAt + this.limits.offerMs;
  }
  private materializeOffer(state: ControlState, request: FloorRequest): void {
    ...
    if (task) {
      task.status = 'completed'; task.statusMessage = 'The floor offer is ready to claim.'; task.updatedAt = request.updatedAt;
  ...
  private statusIn(state: ControlState, request: FloorRequest): Record<string, unknown> {
  ...
  private ownRequest(state: ControlState, handle: string, ticketId: string): FloorRequest {
    const actor = this.actor(state, handle);
  ...
  private held(state: ControlState, handle: string, floorId: string, fence: string): FloorRequest {
  ...
  async requestResourceFloor(handle: string, resourceTypeOrId: ResourceType | string, maybeResourceId?: string, deliveryMode: 'ticket' | 'task' = 'ticket'): Promise<Record<string, unknown>> {
    this.sweep();
    ...
    return this.writers.run(actor.projectId, async () => {
      ...
      const ticketId = this.control.update(state => {
        ...
        const now = this.clock.now();
        const request: FloorRequest = { id: uid(), projectId: actor.projectId, resourceId, resourceType, identityId: actor.identityId, instanceId: actor.instanceId,
  ...
  async requestProjectFloor(handle: string, purpose: FloorRequest['purpose'], deliveryMode: 'ticket' | 'task' = 'ticket'): Promise<Record<string, unknown>> {
    ...
    return this.writers.run(actor.projectId, async () => {
      const ticketId = this.control.update(state => {
        ...
        const now = this.clock.now();
        const request: FloorRequest = { id: uid(), projectId: actor.projectId, resourceId, resourceType: 'project', purpose, identityId: actor.identityId,
  ...
  async claimFloor(handle: string, offerId: string, limit: number, noteCursor?: string): Promise<unknown> {
    this.sweep();
    const actor = this.control.view(s => this.actor(s, handle));
    return this.writers.run(actor.projectId, async () => {
      ...
      if (request.resourceType === 'project') {
        ...
        const floor = this.control.update(state => { const current = state.requests[request.id]!; const resource = state.resources[current.resourceId]!;
          resource.fence = increment(resource.fence); current.state = 'HELD'; current.floorId = uid(); current.fence = resource.fence; current.baseRevision = '0';
      ...
      const floor = this.control.update(state => {
        ...
        current.state = 'HELD'; current.floorId = uid(); current.fence = resource.fence;
  ...
  async read(handle: string, floorId: string, fence: string, limit: number, before?: string): Promise<unknown> {
    this.sweep();
    ...
    this.sweep(); this.control.view(s => this.held(s, handle, floorId, fence));
  ...
  async readNote(handle: string, floorId: string, fence: string, cursor?: string): Promise<unknown> {
    this.sweep();
    const request = this.control.view(s => this.held(s, handle, floorId, fence));
  ...
  async readFloor(handle: string, floorId: string, fence: string, cursor?: string): Promise<unknown> {
    const request = this.control.view(s => this.held(s,handle,floorId,fence));
    if (request.resourceType === 'thread') return this.read(handle,floorId,fence,20,cursor);
    if (request.resourceType === 'note') return this.readNote(handle,floorId,fence,cursor);
    throw new BassfishError('RESOURCE_TYPE_MISMATCH','Project floors have operation-specific readers.');
  }
  async noteOutline(handle: string, floorId: string, fence: string): Promise<unknown> {
    this.sweep(); const request = this.control.view(s => this.held(s,handle,floorId,fence));
  ...
  async findNote(handle: string, floorId: string, fence: string, query: string, mode: 'literal'|'regex', limit: number): Promise<unknown> {
    this.sweep(); const request = this.control.view(s => this.held(s,handle,floorId,fence));
    requireThat(request.resourceType === 'note','RESOURCE_TYPE_MISMATCH','This floor does not belong to a note.');
  ...
  releaseFloor(handle: string, floorId: string, fence: string): unknown {
    this.sweep();
    return this.control.update(state => {
  ...
  cancelFloorRequest(handle: string, ticketId: string): unknown {
    this.sweep();
    return this.control.update(state => {
      const actor = this.actor(state, handle);
  ...
  async waitForFloor(handle: string, ticketId: string, timeout: number, signal?: AbortSignal): Promise<unknown> {
    ...
    while (true) {
      ...
      try { await delay(Math.min(100, Math.max(1, until - performance.now())), undefined, { signal }); }
      catch { try { this.cancelFloorRequest(handle, ticketId); } catch { /* Claim may already have won. */ } throw new BassfishError('CANCELLED', 'The wait was cancelled; inspect ticket state.'); }
    }
  }
  ...
  async resourceHistory(handle: string, floorId: string, fence: string, offset: number, limit: number): Promise<unknown> {
    this.sweep(); const request = this.control.view(s => this.held(s, handle, floorId, fence));
  ...
  async resourceAt(handle: string, floorId: string, fence: string, revision: string, limit: number, cursor?: string): Promise<unknown> {
    this.sweep(); const request = this.control.view(s => this.held(s, handle, floorId, fence));
  ...
  async diffResource(handle: string, floorId: string, fence: string, revision: string): Promise<unknown> {
    this.sweep(); const request = this.control.view(s => this.held(s, handle, floorId, fence));
  ...
  async previewRestore(handle: string, floorId: string, fence: string, revision: string): Promise<unknown> {
    this.sweep(); const request = this.control.view(s => this.held(s, handle, floorId, fence));
  ...
  async restoreRevision(handle: string, floorId: string, fence: string, token: string): Promise<MutationResult> {
    ...
    return this.writers.run(actor.projectId, async () => {
      ...
      this.control.update(state => { const current = this.held(state,handle,floorId,fence); current.state = 'COMMITTING'; state.pending[operation.id] = {
  ...
  private projectHeld(state: ControlState, handle: string, floorId: string, fence: string, purpose?: FloorRequest['purpose']): FloorRequest {
    const request = this.held(state,handle,floorId,fence); requireThat(request.resourceType === 'project','RESOURCE_TYPE_MISMATCH','A project floor is required.');
    requireThat(!purpose || request.purpose === purpose,'PROJECT_PURPOSE_MISMATCH','This project floor was acquired for another purpose.'); return request;
  }
  async projectSnapshotInfo(handle: string, floorId: string, fence: string): Promise<unknown> {
  ...
  async projectSearch(handle: string, floorId: string, fence: string, query: string, states: string[], limit: number, cursor?: string): Promise<unknown> {
    this.sweep(); const request = this.control.view(s => this.projectHeld(s,handle,floorId,fence,'search'));
    ...
    this.sweep(); this.control.view(s => this.projectHeld(s,handle,floorId,fence,'search'));
  ...
  async projectHistorySearch(handle: string, floorId: string, fence: string, query: string, noteId: string | undefined, states: string[], limit: number, cursor?: string): Promise<unknown> {
    this.sweep(); const request = this.control.view(s => this.projectHeld(s,handle,floorId,fence,'search'));
    requireThat(this.searchIndex,'SEARCH_UNAVAILABLE','The rebuildable search index is unavailable.');
  ...
  async projectHistoryList(handle: string, floorId: string, fence: string, limit: number, cursor?: string): Promise<unknown> {
    this.sweep(); const request = this.control.view(s => this.projectHeld(s,handle,floorId,fence,'restore'));
    ...
    this.sweep(); this.control.view(s => this.projectHeld(s,handle,floorId,fence,'restore'));
  ...
  async previewProjectRestore(handle: string, floorId: string, fence: string, targetCommit: string, limit: number, cursor?: string): Promise<unknown> {
    this.sweep(); const request = this.control.view(s => this.projectHeld(s,handle,floorId,fence,'restore')); const actor = this.control.view(s => this.actor(s,handle));
    ...
    const page = built.changes.slice(offset,offset+limit); const previewToken = this.signRestore({ kind: 'snapshotRestore', floorId, fence, projectId: request.projectId, currentCommit: current.commit, targetCommit: target.commit, digest: built.digest, expiresAt: request.expiresAt });
    this.sweep(); this.control.view(s => this.projectHeld(s,handle,floorId,fence,'restore'));
  ...
  async restoreProject(handle: string, floorId: string, fence: string, token: string): Promise<ProjectRestoreResult> {
    this.sweep(); const payload = this.verifyRestore(token); const actor = this.control.view(s => this.actor(s,handle));
    return this.writers.run(actor.projectId,async () => {
  ...
  async exportProject(handle: string, floorId: string, fence: string): Promise<unknown> {
    this.sweep(); const request = this.control.view(s => this.projectHeld(s,handle,floorId,fence,'export')); requireThat(this.dataDir,'EXPORT_UNAVAILABLE','Export storage is unavailable.');
    ...
    this.sweep(); this.control.update(s => { const current = this.projectHeld(s,handle,floorId,fence,'export'); this.finish(current,'RELEASED',s); this.promote(s); }); return result;
  }
  async commitFloor(handle: string, floorId: string, fence: string, base: string, mutation: Mutation): Promise<MutationResult> {
    this.sweep();
    const actor = this.control.view(s => this.actor(s, handle));
    return this.writers.run(actor.projectId, async () => {
      this.sweep();
  ...
  inspect(): unknown { this.sweep(); return this.control.view(s => ({ epoch: this.epoch, projects: values(s.projects), floors: values(s.requests).map(r => ({ ...this.statusIn(s, r), projectId: r.projectId, floorId: r.floorId, identityId: r.identityId, instanceId: r.instanceId })) })); }
  ...
  forceRelease(floorId: string): void {
    this.sweep();
    this.control.update(state => {
      const request = values(state.requests).find(r => r.floorId === floorId);
  ...
  async call(handle: string, name: string, input: unknown, signal?: AbortSignal, options: { taskCapable?: boolean } = {}): Promise<unknown> {
    ...
    switch (name as ToolName) {
      ...
      case 'requestFloor': {
        const target = args.target as { type: ResourceType; id?: string; purpose?: FloorRequest['purpose'] };
      ...
      case 'getFloorRequest': return this.status(handle,args.requestId as string);
      case 'waitForFloor': return this.waitForFloor(handle,args.requestId as string,args.timeoutMs as number,signal);
      case 'cancelFloorRequest': return this.cancelFloorRequest(handle,args.requestId as string);
      case 'claimFloor': return this.claimFloor(handle,args.offerId as string,20);
      case 'readFloor': { const credential = args.floor as { id: string; fencingToken: string }; return this.readFloor(handle,credential.id,credential.fencingToken,args.cursor as string | undefined); }
      case 'releaseFloor': { const credential = args.floor as { id: string; fencingToken: string }; return this.releaseFloor(handle,credential.id,credential.fencingToken); }
      case 'commitFloor': { const credential = args.floor as { id: string; fencingToken: string }; return this.commitFloor(handle,credential.id,credential.fencingToken,args.baseRevision as string,toMutation(args.mutation as Record<string,unknown>)); }
      case 'getNoteOutline': { const credential = args.floor as { id: string; fencingToken: string }; return this.noteOutline(handle,credential.id,credential.fencingToken); }
      case 'findInNote': { const credential = args.floor as { id: string; fencingToken: string }; return this.findNote(handle,credential.id,credential.fencingToken,args.query as string,args.mode as 'literal'|'regex',args.limit as number); }
      case 'inspectSnapshot': { const credential = args.floor as { id: string; fencingToken: string }; return this.projectSnapshotInfo(handle,credential.id,credential.fencingToken); }
      case 'searchProjectNotes': { const credential = args.floor as { id: string; fencingToken: string }; return this.projectSearch(handle,credential.id,credential.fencingToken,args.query as string,args.states as string[],args.limit as number,args.cursor as string | undefined); }
      case 'searchProjectNoteHistory': { const credential = args.floor as { id: string; fencingToken: string }; return this.projectHistorySearch(handle,credential.id,credential.fencingToken,args.query as string,args.noteId as string | undefined,args.states as string[],args.limit as number,args.cursor as string | undefined); }
      case 'exportSnapshot': { const credential = args.floor as { id: string; fencingToken: string }; return this.exportProject(handle,credential.id,credential.fencingToken); }
      case 'listSnapshotHistory': { const credential = args.floor as { id: string; fencingToken: string }; return this.projectHistoryList(handle,credential.id,credential.fencingToken,args.limit as number,args.cursor as string | undefined); }
      case 'previewSnapshotRestore': { const credential = args.floor as { id: string; fencingToken: string }; return this.previewProjectRestore(handle,credential.id,credential.fencingToken,args.targetCommit as string,args.limit as number,args.cursor as string | undefined); }
      case 'restoreSnapshot': { const credential = args.floor as { id: string; fencingToken: string }; return this.restoreProject(handle,credential.id,credential.fencingToken,args.previewToken as string); }
      case 'listHistory': { const credential = args.floor as { id: string; fencingToken: string }; return this.resourceHistory(handle,credential.id,credential.fencingToken,args.offset as number,args.limit as number); }
      case 'readRevision': { const credential = args.floor as { id: string; fencingToken: string }; return this.resourceAt(handle,credential.id,credential.fencingToken,args.revision as string,20,args.cursor as string | undefined); }
      case 'diffRevision': { const credential = args.floor as { id: string; fencingToken: string }; return this.diffResource(handle,credential.id,credential.fencingToken,args.revision as string); }
      case 'previewRestore': { const credential = args.floor as { id: string; fencingToken: string }; return this.previewRestore(handle,credential.id,credential.fencingToken,args.revision as string); }
      case 'restoreRevision': { const credential = args.floor as { id: string; fencingToken: string }; return this.restoreRevision(handle,credential.id,credential.fencingToken,args.previewToken as string); }
    }
  }
}
