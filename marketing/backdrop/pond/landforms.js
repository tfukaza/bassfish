...
export function roundedRockGeometry(segments = 32, rings = 20) {
  ...
  for (let i = 0; i < p.count; i += 3) {
    ...
    if (planes.some(plane => plane.normal.dot(normal) > 0.9999)) continue;
  ...
  for (let i = 0; i < positions.count; i++) {
    ...
    const distances = planes
      .filter(plane => plane.normal.dot(direction) > 0.05)
      .map(plane => plane.distance / plane.normal.dot(direction));
    const nearest = Math.min(...distances),
      radius =
        nearest -
        0.085 * Math.log(distances.reduce((sum, d) => sum + Math.exp((nearest - d) / 0.085), 0));
    const point = direction.clone().multiplyScalar(radius);
    point.x *= 1 + 0.055 * direction.y - 0.035 * direction.z;
    point.y = point.y * 0.73 + point.x * 0.04;
    point.z *= 0.96 + 0.045 * direction.x;
  ...
  for (let i = 0; i < positions.count; i++) {
    ...
    group.normal.add(new THREE.Vector3().fromBufferAttribute(normals, i));
    group.indices.push(i);
  }
  for (const group of groups.values()) {
    group.normal.normalize();
    for (const i of group.indices)
      normals.setXYZ(i, group.normal.x, group.normal.y, group.normal.z);
  }
  return geometry;
}
...
export function roundedBank(shore, curve) {
  const steps = 12,
    ...
    blend = [],
    indices = [];
  const widths = shore.map(
    (_, i) =>
      0.6 +
      0.065 * Math.sin((i / count) * Math.PI * 5 + 0.7) +
      0.025 * Math.sin((i / count) * Math.PI * 11),
  );
  for (let i = 0; i < count; i++) {
    const tangent = curve.getTangent(i / (count - 1)),
      row = [];
    for (let j = 0; j <= steps; j++) {
      const u = j / steps,
        distance = widths[i] * u;
      const point = new THREE.Vector3(
        shore[i].x + tangent.z * distance,
        0.06 + 0.32 * smooth(u),
        shore[i].z - tangent.x * distance,
      );
      if (i === 0) point.z = POND_HALF;
      if (i === count - 1) point.x = POND_HALF;
      row.push(point);
      positions.push(...point.toArray());
      uvs.push((point.x + POND_HALF) / 3, (POND_HALF - point.z) / 3);
      grassUvs.push((point.x + POND_HALF) / 8, (POND_HALF - point.z) / 8);
      const transition = THREE.MathUtils.clamp((u - 0.5 - 0.035 * Math.sin(i * 0.12)) / 0.39, 0, 1);
      blend.push(smooth(transition));
      if (i < count - 1 && j < steps) {
        const k = i * (steps + 1) + j;
  ...
  for (let i = 0; i < count; i++)
    geometry.attributes.normal.setXYZ(i * (steps + 1) + steps, 0, 1, 0);

  const crest = rows.map(row => row[steps]);
  const rim = [
    new THREE.Vector3(-POND_HALF, 0.38, POND_HALF),
    ...rows[0].slice().reverse(),
    ...shore.slice(1).map(p => new THREE.Vector3(p.x, 0.06, p.z)),
    ...rows.at(-1).slice(1),
    new THREE.Vector3(POND_HALF, 0.38, -POND_HALF),
    new THREE.Vector3(-POND_HALF, 0.38, -POND_HALF),
  ];
  ...
  for (let i = 0; i < rim.length; i++) {
    ...
    const bottomA = [a.x, -4.16, a.z],
      bottomB = [b.x, -4.16, b.z];
    wallPositions.push(
      ...a.toArray(),
      ...bottomA,
      ...b.toArray(),
  ...
  function heightAt(x, z) {
    ...
    for (let i = 0; i < count - 1; i++) {
      ...
      if (squared < nearest) {
        nearest = squared;
        distance = (px * dz - pz * dx) / Math.sqrt(lengthSq);
        width = THREE.MathUtils.lerp(widths[i], widths[i + 1], t);
      }
    }
    return 0.06 + 0.32 * smooth(THREE.MathUtils.clamp(distance / width, 0, 1));
  }
...
export function bankMaterial(sandMap, grassMap) {
  ...
  material.onBeforeCompile = shader => {
    ...
    shader.vertexShader =
      'attribute vec2 grassUv;attribute float grassBlend;varying vec2 vGrassUv;varying float vGrassBlend;\n' +
      shader.vertexShader;
    ...
    shader.fragmentShader =
      'uniform sampler2D uBankGrass;varying vec2 vGrassUv;varying float vGrassBlend;\n' +
      shader.fragmentShader;
    shader.fragmentShader = shader.fragmentShader.replace(
      '#include <map_fragment>',
      'diffuseColor *= mix(texture2D(map,vMapUv),texture2D(uBankGrass,vGrassUv),vGrassBlend);',
    );
  };
  return material;
}
