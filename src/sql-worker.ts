...
export interface SqlEndpoint { socketPath: string; password: string }
...
export async function runSqlWorker(dataDir: string, binary: string): Promise<void> {
  ...
  try { password = (await readFile(passwordPath, 'utf8')).trim(); }
    ...
    password = randomBytes(32).toString('hex'); await writeFile(passwordPath, password, { mode: 0o600, flag: 'wx' });
  }
  requireThat(/^[a-f0-9]{64}$/.test(password), 'SQL_CREDENTIALS', 'The SQL credential file is incomplete or invalid; no listener was started.');
  ...
  const doltEnv = { ...process.env, DOLT_ROOT_PATH: configRoot, DOLT_CLI_PASSWORD: undefined,
    DOLT_ROOT_PASSWORD: password, DOLT_ROOT_HOST: 'localhost' };
  ...
  const config = [
    `data_dir: ${yamlString(projects)}`, `cfg_dir: ${yamlString(configRoot)}`,
    `privilege_file: ${yamlString(privileges)}`, `branch_control_file: ${yamlString(join(configRoot, 'branch-control.db'))}`,
    'log_level: error', 'behavior:', '  autocommit: true', '  dolt_transaction_commit: false', '  event_scheduler: "OFF"',
    '  auto_gc_behavior:', '    enable: false', 'listener:', '  host: "127.0.0.1"', `  port: ${port}`,
    `  socket: ${yamlString(socketPath)}`, '  read_timeout_millis: 60000', '  write_timeout_millis: 60000',
    'system_variables:', `  secure_file_priv: ${yamlString(join(run, 'disabled-file-access'))}`, '',
  ].join('\n');
  const configPath = join(run, 'sql.yaml');
  await writeFile(configPath, config, { mode: 0o600 });
  const child = spawn(binary, ['sql-server', '--config', configPath], { cwd: dataDir,
    env: doltEnv, stdio: ['ignore', 'pipe', 'pipe'] });
  let stopping = false;
  const stop = () => {
    if (stopping) return;
    stopping = true; child.kill('SIGTERM');
    const hard = setTimeout(() => child.kill('SIGKILL'), 5000); hard.unref();
  };
  ...
  child.stdout.on('data', () => { /* Do not log SQL or content. */ });
  child.stderr.on('data', data => process.stderr.write(data));
  child.once('error', error => { process.stderr.write(`Dolt could not start: ${error.message}\n`); unlock(); process.exit(1); });
  child.once('exit', code => { unlock(); process.exit(stopping ? 0 : code || 1); });
  const socketMode = setInterval(() => { void chmod(socketPath, 0o600).catch(() => {}); }, 1000); socketMode.unref();
}
