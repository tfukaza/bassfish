...
export function mountPond(container, options = {}) {
  ...
  let reducedMotion = motion.matches,
    paused = options.paused ?? reducedMotion,
    disposed = false,
  ...
  const renderer = new THREE.WebGLRenderer({
    antialias: true,
    alpha: Boolean(options.createStory),
    powerPreference: 'low-power',
  });
  renderer.setPixelRatio(Math.min(devicePixelRatio || 1, 1.65));
  renderer.setClearColor(0xffffff, options.createStory ? 0 : 1);
  ...
  scene.add(world);
  const light = new THREE.DirectionalLight(0xfff7e8, 2.5);
  light.position.set(-4, 14, 6);
  ...
  const fill = new THREE.DirectionalLight(0xcbe5df, 0.65);
  ...
  for (const objects of batches.values()) {
    ...
    const combined = new THREE.Mesh(g, objects[0].material);
  ...
  function draw(dt = 0) {
    ...
    elevation = THREE.MathUtils.lerp(elevation, targetElevation, blend);
    const r = 21,
      focusY = -1.5875;
    camera.position.set(
      Math.sin(yaw) * Math.cos(elevation) * r,
      Math.sin(elevation) * r + focusY,
      Math.cos(yaw) * Math.cos(elevation) * r,
    );
    camera.lookAt(0, focusY, 0);
    timeUniform.value = time;
    if (!story)
      fishes.forEach((f, i) => {
        const a = time * (0.1 + i * 0.013) + f.phase;
        let dx, dz;
        if (i === 0) {
          const along = Math.cos(a) * 0.8,
            across = Math.sin(a) * 0.1;
          f.fish.position.set(
            1.1 + along * 0.75 + across * 0.66,
            -1.63 + Math.sin(time * 0.5 + f.phase) * 0.05,
            layoutZ(2.72 - along * 0.66 + across * 0.75),
          );
          dx = -Math.sin(a) * 0.8 * 0.75 + Math.cos(a) * 0.1 * 0.66;
          dz = Math.sin(a) * 0.8 * 0.66 + Math.cos(a) * 0.1 * 0.75;
        } else {
          f.fish.position.set(
            3.17 + Math.cos(a) * 0.58,
            -1.83 + Math.sin(time * 0.5 + f.phase) * 0.05,
            layoutZ(1.32 + Math.sin(a) * 0.3),
          );
          dx = -Math.sin(a) * 0.58;
          dz = Math.cos(a) * 0.3;
        }
        f.fish.rotation.set(
          0,
          Math.atan2(-dz * DEPTH_SCALE, dx),
          Math.sin(time * 0.7 + f.phase) * 0.022,
        );
      });
  ...
  function resize() {
    ...
    const aspect = width / height,
      view = (aspect < 0.8 ? 14.2 / aspect : aspect < 1.25 ? 16.9 : 15.5) * 1.16;
    ...
    renderer.setPixelRatio(
      Math.min(devicePixelRatio || 1, options.pixelRatio ?? 1.65, width < 700 ? 1.4 : 1.65),
    );
  ...
  let dragging = false,
    ...
    lastY = 0;
  const onDown = e => {
    if (options.createStory || e.button !== 0) return;
    dragging = true;
    moved = 0;
    lastX = e.clientX;
    lastY = e.clientY;
  ...
  const onMove = e => {
    ...
    lastX = e.clientX;
    lastY = e.clientY;
    targetYaw = clamp(targetYaw - dx * 0.005, 0.15, 1.39);
    targetElevation = clamp(targetElevation + dy * 0.004, 0.25, 0.88);
    if (paused) draw();
  };
  function signal() {
    signalAge = paused ? 0.5 : 0;
    draw();
  }
  const onUp = e => {
    if (!dragging) return;
  ...
  const onMotion = () => {
    ...
    if (reducedMotion) {
      paused = true;
      draw();
      container.dispatchEvent(new Event('pond:motionchange'));
    }
  };
  ...
  raf = requestAnimationFrame(tick);
  return {
    ready: Promise.all([habitat.ready, bass.ready]).then(() => {
      if (!disposed) {
        texturesReady = true;
        habitat.update(time, signalAge);
        story = options.createStory?.({
          scene,
          world,
          camera,
          fishes,
    ...
    setStoryProgress(value) {
      storyProgress = clamp(value, 0, options.storyEnd ?? 5.8);
      if (!disposed) draw();
    },
    resetView() {
      targetYaw = 0.72;
    ...
    dispose() {
      ...
      story?.dispose();
      const geometries = new Set(retiredGeometries),
        materials = new Set(materialCache.values());
      scene.traverse(obj => {
