...
test('offers reveal no content, claims are exclusive, one commit releases to FIFO successor', async t => {
  ...
  const first = await s.requestResourceTurn(a.agentHandle, thread), second = await s.requestResourceTurn(b.agentHandle, thread);
  ...
  await assert.rejects(s.requestResourceTurn(a.agentHandle, thread), errorCode('TURN_REQUEST_EXISTS'));
  await assert.rejects(s.claimTurn(b.agentHandle, first.offerId as string, 20), errorCode('NOT_TURN_OWNER'));
  ...
  await assert.rejects(s.read(b.agentHandle, turn.turn.id, turn.turn.fencingToken, 20), errorCode('NOT_TURN_OWNER'));
  await assert.rejects(s.commitTurn(a.agentHandle, turn.turn.id, turn.turn.fencingToken, '0', { kind: 'appendMessage', body: 'wrong' }), errorCode('REVISION_CHANGED'));
  const result = await s.commitTurn(a.agentHandle, turn.turn.id, turn.turn.fencingToken, turn.snapshot.revision, { kind: 'appendMessage', body: 'hello' });
  ...
  await assert.rejects(s.commitTurn(a.agentHandle, turn.turn.id, turn.turn.fencingToken, turn.snapshot.revision, { kind: 'appendMessage', body: 'duplicate' }), errorCode('STALE_TURN'));
  const next = await s.claimTurn(b.agentHandle, s.status(b.agentHandle, second.requestId as string).offerId as string, 20) as Turn;
  assert.deepEqual(next.page.messages.map(m => m.body), ['hello']); assert.equal(next.turn.fencingToken, '2');
...
test('unclaimed offers expire, cancellation is explicit, wait timeout preserves ticket', async t => {
  const f = await fixture(); t.after(f.close); const { service: s, a, b, thread, clock } = f;
  const first = await s.requestResourceTurn(a.agentHandle, thread), next = await s.requestResourceTurn(b.agentHandle, thread);
  assert.equal((await s.waitForTurn(b.agentHandle, next.requestId as string, 1) as { state: string }).state, 'queued');
  clock.advance(30_000);
  await assert.rejects(s.claimTurn(a.agentHandle, first.offerId as string, 20), errorCode('OFFER_EXPIRED'));
  assert.equal(s.status(b.agentHandle, next.requestId as string).state, 'offered');
  s.cancelTurnRequest(b.agentHandle, next.requestId as string);
  ...
  const waiter = await s.requestResourceTurn(b.agentHandle, thread); const controller = new AbortController();
  const waiting = s.waitForTurn(b.agentHandle, waiter.requestId as string, 100, controller.signal); controller.abort();
  ...
  s.releaseTurn(a.agentHandle, claimed.turn.id, claimed.turn.fencingToken);
});
...
test('simultaneous claims and mutations have exactly one winner', async t => {
  ...
  const claims = await Promise.allSettled(Array.from({ length: 8 }, () => s.claimTurn(a.agentHandle, offer.offerId as string, 20)));
  assert.equal(claims.filter(r => r.status === 'fulfilled').length, 1);
  const turn = (claims.find(r => r.status === 'fulfilled') as PromiseFulfilledResult<Turn>).value;
  const commits = await Promise.allSettled(Array.from({ length: 8 }, () => s.commitTurn(a.agentHandle, turn.turn.id, turn.turn.fencingToken, turn.snapshot.revision, { kind: 'appendMessage', body: 'once' })));
...
test('accepted COMMITTING reservation survives deadline, disconnect, cancellation and force-release', async t => {
  const f = await fixture(); t.after(f.close); const { service: s, a, b, thread, clock } = f;
  const turn = await hold(s, a.agentHandle, thread); const queued = await s.requestResourceTurn(b.agentHandle, thread);
  ...
  const writing = s.commitTurn(a.agentHandle, turn.turn.id, turn.turn.fencingToken, turn.snapshot.revision, { kind: 'appendMessage', body: 'durable' });
  await began; clock.advance(31_000); s.sweep(); s.disconnect(a.agentHandle, false);
  assert.throws(() => s.forceRelease(turn.turn.id), errorCode('NOT_CLAIMED'));
...
test('lost commit reply resolves without replay; absent fails; unknown protects every project write', async t => {
  for (const failure of ['after_commit', 'absent', 'unknown'] as const) await t.test(failure, async t => {
    const f = await fixture(); t.after(f.close); const { service: s, a, b, thread } = f;
    const turn = await hold(s, a.agentHandle, thread); const queued = await s.requestResourceTurn(b.agentHandle, thread); f.content.fail = failure;
    const writing = s.commitTurn(a.agentHandle, turn.turn.id, turn.turn.fencingToken, turn.snapshot.revision, { kind: 'appendMessage', body: 'one write' });
...
test('restart persists identities and FIFO; stale instances and turns never regain ownership', async t => {
  ...
  await assert.rejects(restarted.read(a.agentHandle, turn.turn.id, turn.turn.fencingToken, 20), errorCode('SESSION_EXPIRED'));
  ...
  await assert.rejects(restarted.read(a2.agentHandle, turn.turn.id, turn.turn.fencingToken, 20), errorCode('NOT_TURN_OWNER'));
  ...
  await assert.rejects(restarted.requestResourceTurn(otherRepo.agentHandle, thread), errorCode('NOT_FOUND'));
});

test('reconnect grace retains order; clean disconnect cancels; clock discontinuity revokes claimed turns', async t => {
  const f = await fixture(); t.after(f.close); const { service: s, a, b, thread, clock } = f;
  const turn = await hold(s, a.agentHandle, thread); const queued = await s.requestResourceTurn(b.agentHandle, thread);
  s.disconnect(b.agentHandle, false); s.releaseTurn(a.agentHandle, turn.turn.id, turn.turn.fencingToken);
  const c = await s.open('/repo/.git', 'Carol'); const third = await s.requestResourceTurn(c.agentHandle, thread);
  ...
  const next = await s.claimTurn(c.agentHandle, s.status(c.agentHandle, third.requestId as string).offerId as string, 20) as Turn;
  clock.jumped = true; await assert.rejects(s.read(c.agentHandle, next.turn.id, next.turn.fencingToken, 20), errorCode('TURN_EXPIRED'));
});
...
test('archive and restore serialize with appends; pagination stays within the claimed snapshot', async t => {
  ...
  for (let n = 1; n <= 4; n++) {
    ...
    await s.commitTurn(a.agentHandle, turn.turn.id, turn.turn.fencingToken, turn.snapshot.revision, { kind: 'appendMessage', body: `Message ${n}` });
  }
  const offer = await s.requestResourceTurn(a.agentHandle, thread); const turn = await s.claimTurn(a.agentHandle, offer.offerId as string, 2) as Turn;
  assert.deepEqual(turn.page.messages.map(m => m.sequence), ['3', '4']); assert.equal(turn.nextCursor, '3');
  const earlier = await s.read(a.agentHandle, turn.turn.id, turn.turn.fencingToken, 2, '3') as { page: { messages: { sequence: string }[] } };
  assert.deepEqual(earlier.page.messages.map(m => m.sequence), ['1', '2']);
  await s.commitTurn(a.agentHandle, turn.turn.id, turn.turn.fencingToken, turn.snapshot.revision, { kind: 'archiveThread' });
  const archived = await hold(s, a.agentHandle, thread);
  await assert.rejects(s.commitTurn(a.agentHandle, archived.turn.id, archived.turn.fencingToken, archived.snapshot.revision, { kind: 'appendMessage', body: 'blocked' }), errorCode('RESOURCE_ARCHIVED'));
  await s.commitTurn(a.agentHandle, archived.turn.id, archived.turn.fencingToken, archived.snapshot.revision, { kind: 'activateThread' });
...
test('message retraction and reinstatement are revisioned turn mutations', async t => {
  ...
  const appended = await s.commitTurn(a.agentHandle,turn.turn.id,turn.turn.fencingToken,turn.snapshot.revision,{ kind: 'appendMessage', body: 'retain me' });
  turn = await hold(s,a.agentHandle,thread);
  await s.commitTurn(a.agentHandle,turn.turn.id,turn.turn.fencingToken,turn.snapshot.revision,{ kind: 'retractMessage', messageId: appended.messageId! });
  turn = await hold(s,a.agentHandle,thread);
  assert.equal(turn.page.messages[0]!.retracted,true); assert.equal(turn.page.messages[0]!.body,'');
  await s.commitTurn(a.agentHandle,turn.turn.id,turn.turn.fencingToken,turn.snapshot.revision,{ kind: 'reinstateMessage', messageId: appended.messageId! });
  turn = await hold(s,a.agentHandle,thread);
  assert.equal(turn.page.messages[0]!.retracted,false); assert.equal(turn.page.messages[0]!.body,'retain me');
});
