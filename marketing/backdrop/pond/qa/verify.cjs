...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => window.bassfishPond?.getStats().frames > 4);
    check(
      'Three.js pond renders',
      (await stats()).threeRevision === '180' && (await stats()).triangles > 0,
    );
    check(
      'Generated terrain, rock, and foliage textures are ready',
      (await stats()).texturesReady &&
        (await stats()).terrainTextures === 4 &&
        (await stats()).texturedRocks > 0 &&
        (await stats()).foliageTextures === 6,
    );
    check(
      'Both swimming bass use the generated body and fin textures',
      (await stats()).texturesReady &&
        (await stats()).texturedBass === 2 &&
        (await stats()).fishTextures === 2,
    );
    check(
      'Fuller bank planting uses billboards and lily leaves stay flat',
      (await stats()).groundPlants === 32 &&
        (await stats()).billboards === 39 &&
        (await stats()).lilyPads === 5,
    );
    const alphaChecks = await page.evaluate(async () => {
      ...
      return Promise.all(
        names.map(async name => {
          const blob = await (await fetch(`./foliage/${name}-v2.png`)).blob(),
    ...
    check('Signal appears while paused', (await stats()).signalAge === 0.5);
    await page.screenshot({ path: path.join(__dirname, 'signal.png') });
    await page.mouse.move(760, 460);
    await page.mouse.down();
    await page.mouse.move(950, 520, { steps: 8 });
    await page.mouse.up();
    check('Drag changes camera angle', Math.abs((await stats()).yaw - 0.72) > 0.2);
    await page.screenshot({ path: path.join(__dirname, 'rotated.png') });
    await page.getByRole('button', { name: 'Reset view', exact: true }).click();
    const reset = await stats();
    check('Reset restores isometric view', reset.yaw === 0.72 && reset.elevation === 0.56);
    ...
    check(
      'Pause control reflects motion preference',
      await page.getByRole('button', { name: 'Play animation', exact: true }).isVisible(),
    );
    await page.reload();
    await page.waitForFunction(() => !!window.bassfishPond);
    ...
    await page.screenshot({
      path: path.join(__dirname, '..', 'poster.jpg'),
      type: 'jpeg',
      quality: 90,
    });
    const rendererStats = await stats();
    await page.evaluate(() => window.bassfishPond.dispose());
    const count = (await stats()).frames;
    await page.waitForTimeout(200);
    check(
      'Dispose removes canvas and stops drawing',
      (await page.locator('canvas').count()) === 0 && (await stats()).frames === count,
    );
    ...
    check(
      'WebGL failure shows the still fallback',
      await fallback.locator('#fallback').isVisible(),
    );
    ...
    check(
      'Texture failure keeps the poster and disposes the canvas',
      (await failedTexture.locator('canvas').count()) === 0,
    );
    ...
    check(
      'Rock texture failure retains the poster and disposes the canvas',
      (await failedRock.locator('canvas').count()) === 0,
    );
    ...
    check(
      'Foliage load failure retains the poster and disposes the canvas',
      (await failedFoliage.locator('canvas').count()) === 0,
    );
    ...
    check(
      'Fish texture load failure retains the poster and disposes the canvas',
      (await failedFish.locator('canvas').count()) === 0,
    );
