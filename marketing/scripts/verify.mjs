...
import {C,workflowGeometry,DURATION,FPS,GIF_DURATION,VIDEO_WIDTH,VIDEO_HEIGHT,GIF_WIDTH,GIF_HEIGHT,README_GIF_WIDTH,README_GIF_HEIGHT,README_GIF_FPS} from './artwork.mjs';
...
assert.equal(readmeGif.width,README_GIF_WIDTH);assert.equal(readmeGif.pageHeight,README_GIF_HEIGHT);assert.equal(readmeGif.loop,0);assert.equal(readmeGif.pages,DURATION*README_GIF_FPS);assert(Math.abs(readmeGif.delay.reduce((a,b)=>a+b,0)-DURATION*1000)<100);
const {data:readmeFrame,info:readmeFrameInfo}=await sharp(readmeGifPath,{page:Math.floor(readmeGif.pages/2)}).ensureAlpha().raw().toBuffer({resolveWithObject:true});
assert.equal(readmeFrame[(Math.floor(readmeFrameInfo.height/2)*readmeFrameInfo.width+Math.floor(readmeFrameInfo.width/2))*4+3],0);
checks.push(`README GIF: ${README_GIF_WIDTH} × ${README_GIF_HEIGHT}, ${DURATION*README_GIF_FPS} frames, ${DURATION} seconds, transparent canvas`);
