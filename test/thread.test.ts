...
const credential = (turn: { turn: { id: string; fencingToken: string } }) => ({ id: turn.turn.id, fencingToken: turn.turn.fencingToken });
async function commit(f: Fixture, handle: string, threadId: string, mutation: Record<string, unknown>): Promise<{ revision: string }> {
  ...
  return f.service.call(handle, 'commitTurn', { turn: credential(turn), baseRevision: turn.snapshot.revision, mutation }) as Promise<{ revision: string }>;
}
async function rejects(f: Fixture, handle: string, threadId: string, mutation: Record<string, unknown>, code: string): Promise<void> {
  const turn = await hold(f.service, handle, threadId);
  await assert.rejects(f.service.call(handle, 'commitTurn', { turn: credential(turn), baseRevision: turn.snapshot.revision, mutation }), errorCode(code));
...
test('thread descriptions are edited under the turn, revisioned, and restorable', async t => {
  ...
  const history = await s.call(a.agentHandle, 'listHistory', { turn: credential(turn) }) as { entries: { kind: string }[] };
  assert.deepEqual(history.entries.map(entry => entry.kind).filter(kind => kind === 'setThreadDescription').length, 2);
  const preview = await s.call(a.agentHandle, 'previewRestore', { turn: credential(turn), revision: '1' }) as { previewToken: string };
  await s.call(a.agentHandle, 'restoreRevision', { turn: credential(turn), previewToken: preview.previewToken });
  ...
  const ticket = await s.call(a.agentHandle, 'requestTurn', { target: { type: 'note', id: note.noteId } }) as { offerId: string };
  const noteTurn = await s.call(a.agentHandle, 'claimTurn', { offerId: ticket.offerId }) as { turn: { id: string; fencingToken: string }; snapshot: { revision: string } };
  await assert.rejects(s.call(a.agentHandle, 'commitTurn', { turn: credential(noteTurn), baseRevision: noteTurn.snapshot.revision, mutation: { kind: 'setThreadDescription', description: 'x' } }), errorCode('RESOURCE_TYPE_MISMATCH'));
  await assert.rejects(s.call(a.agentHandle, 'commitTurn', { turn: credential(noteTurn), baseRevision: noteTurn.snapshot.revision, mutation: { kind: 'deleteThread' } }), errorCode('RESOURCE_TYPE_MISMATCH'));
  await s.call(a.agentHandle, 'releaseTurn', { turn: credential(noteTurn) });
});
...
test('deleted threads are a lifecycle state that mirrors deleted notes', async t => {
  ...
  const request = await s.call(a.agentHandle, 'requestTurn', { target: { type: 'project', purpose: 'snapshot' } }) as { offerId: string };
  const project = await s.call(a.agentHandle, 'claimTurn', { offerId: request.offerId }) as { turn: { id: string; fencingToken: string } };
  const info = await s.call(a.agentHandle, 'inspectSnapshot', { turn: credential(project) }) as { threads: { id: string; state: string; description: string }[] };
