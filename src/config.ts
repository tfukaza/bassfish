...
export async function loadRuntimeConfig(dataDir: string): Promise<RuntimeConfig> {
  try { return runtimeConfigSchema.parse(JSON.parse(await readFile(join(dataDir, 'config.json'), 'utf8'))); }
    ...
    if (error instanceof z.ZodError || error instanceof SyntaxError) throw new BassfishError('INVALID_CONFIG', 'Bassfish config.json is invalid or contains unknown keys.');
    throw error;
  }
}
export async function saveRuntimeConfig(dataDir: string, config: RuntimeConfig): Promise<void> {
  const valid = runtimeConfigSchema.parse(config); await mkdir(dataDir, { recursive: true, mode: 0o700 });
  const temporary = join(dataDir, `.config-${process.pid}.tmp`); await writeFile(temporary, JSON.stringify(valid, null, 2) + '\n', { mode: 0o600 });
  await rename(temporary, join(dataDir, 'config.json')); await chmod(join(dataDir, 'config.json'), 0o600);
}

export function dataDirectory(): string {
  if (process.env.BASSFISH_DATA_DIR) return resolve(process.env.BASSFISH_DATA_DIR);
  if (process.platform === 'darwin') return join(homedir(), 'Library', 'Application Support', 'bassfish');
...
export function socketPath(dataDir: string): string {
  ...
  if (Buffer.byteLength(path) > 96) throw new BassfishError('SOCKET_PATH_TOO_LONG', 'Choose a shorter BASSFISH_DATA_DIR (Unix socket path limit, including the longer SQL socket name).');
  return path;
}
