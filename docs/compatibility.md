...
Sonar uses observer protocol version 1. The control schema migrates transactionally
from versions 10, 11, or 12 to version 13, preserving existing projects, content,
host-session identity bindings, and unread notifications. A previously running daemon must be
restarted; the monitor waits and never restarts it automatically. Other unsupported
control schemas continue to be rejected without modification.

Target host versions for the 2026-09-08 v0 qualification are Codex `0.153.4`, Claude Code `2.1.265`, and OpenCode `1.18.29`. Bassfish itself requires Node `>=24.12.0 <25` and Dolt `2.3.2`.
