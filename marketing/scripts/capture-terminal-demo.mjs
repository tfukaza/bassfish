...
const apiAfter = apiBefore.replace(
  '  throw new Error',
...
const clientAfter = clientBefore.replace(
  "return request('/items');",
...
try {
  ...
  const a = await client('api-agent'),
    b = await client('client-agent');
  ...
  const sa = await call(a, 'getContext'),
    sb = await call(b, 'getContext');
  assert.equal(
    sa.agents.some(agent => agent.name === b.name),
    true,
  );
  assert.equal(
    sb.agents.some(agent => agent.name === a.name),
    true,
  );
  const thread = await call(a, 'createResource', {
    resourceType: 'thread',
  ...
  async function claim(agent) {
    ...
    assert.equal(turn.state, 'claimed');
    return turn;
  }
  async function post(agent, turn, body) {
    const result = await call(agent, 'commitTurn', {
      turnToken: turn.turnToken,
      mutation: { kind: 'appendMessage', body },
    });
    return { body, revisionRead: turn.resource.revision, result };
  }
  const proposal = await post(
    a,
    await claim(a),
    'Adding pagination: /items will return { items, total }.',
  );
  const bFirst = await claim(b);
  assert.equal(bFirst.messages.at(-1).body, proposal.body);
  const objection = await post(
    b,
    bFirst,
    'loadItems() expects an array. Keep /items; add /v2/items.',
  );
  const aSecond = await claim(a);
  assert.equal(aSecond.messages.at(-1).body, objection.body);
  const agreement = await post(a, aSecond, 'Agreed. /items stays unchanged. Adding /v2/items.');
  const bSecond = await claim(b);
  assert.equal(bSecond.messages.at(-1).body, agreement.body);
  ...
  const tests = await exec(process.execPath, ['--test', '--test-reporter=spec', 'demo.test.mjs'], {
    cwd: repo,
  });
  ...
  assert.deepEqual(
    final.messages.map(m => m.body),
    [proposal, objection, agreement, followup].map(m => m.body),
  );
  await call(a, 'releaseTurn', { turnToken: final.turnToken });
  const demo = {
    kind: 'Scripted terminal session with captured real Bassfish MCP calls',
    capturedAt: new Date().toISOString(),
    agents: [a.name, b.name],
    thread: 'API pagination',
    messages: final.messages.map(m => ({ name: m.author, body: m.body, sequence: m.sequence })),
    reads: {
      clientFirst: bFirst.messages.map(m => m.body),
      apiSecond: aSecond.messages.map(m => m.body),
      clientSecond: bSecond.messages.map(m => m.body),
    },
    writes: [proposal, objection, agreement, followup].map(p => ({
      revisionRead: p.revisionRead,
      result: p.result,
    })),
    files: { apiBefore, apiAfter, clientBefore, clientAfter, testSource },
    tests: { passed: 3, failed: 0, stdout: tests.stdout },
    transcript: trace,
  };
  ...
  await writeFile(
    join(output, 'session-data.mjs'),
    'export const demo = ' +
      JSON.stringify(
        { messages: demo.messages, writes: demo.writes, tests: demo.tests, files: demo.files },
        null,
        2,
      ) +
      ';\n',
  ...
  console.log(
    `Captured ${trace.length} real MCP calls, ${demo.messages.length} shared messages, and ${demo.tests.passed} passing fixture tests.`,
  );
} finally {
  ...
  try {
    ...
    /* No daemon to stop if startup failed. */
  }
