...
test('offers reveal no content, claims are exclusive, one commit releases to FIFO successor', async t => {
  ...
  const next = (await s.claimTurn(
    b.agentHandle,
    s.status(b.agentHandle, second.requestId as string).offerId as string,
    20,
  )) as Turn;
  assert.deepEqual(
...
test('60-second hard deadline, delayed claim, no renewal, expired content reads rejected after I/O', async t => {
  ...
  s.heartbeat(a.agentHandle);
  await s.read(a.agentHandle, turn.turn.id, turn.turn.fencingToken, 1);
  clock.advance(1);
  assert.throws(
    () => s.releaseTurn(a.agentHandle, turn.turn.id, turn.turn.fencingToken),
  ...
  const next = (await s.claimTurn(
    b.agentHandle,
    s.status(b.agentHandle, queued.requestId as string).offerId as string,
    20,
  )) as Turn;
...
test('unclaimed offers expire, cancellation is explicit, wait timeout preserves ticket', async t => {
  ...
  assert.equal(
    ((await s.waitForTurn(b.agentHandle, next.requestId as string, 1)) as { state: string }).state,
    'queued',
  );
...
test('accepted COMMITTING reservation survives deadline, disconnect, cancellation and force-release', async t => {
  ...
  s.sweep();
  s.disconnect(a.agentHandle, false);
...
test('lost commit reply resolves without replay; absent fails; unknown protects every project write', async t => {
  for (const failure of ['after_commit', 'absent', 'unknown'] as const)
    await t.test(failure, async t => {
      ...
      const turn = await hold(s, a.agentHandle, thread);
      const queued = await s.requestResourceTurn(b.agentHandle, thread);
      f.content.fail = failure;
      const writing = s.commitTurn(
        a.agentHandle,
        turn.turn.id,
        turn.turn.fencingToken,
        turn.snapshot.revision,
        { kind: 'appendMessage', body: 'one write' },
      );
      if (failure === 'after_commit') assert.equal((await writing).sequence, '1');
      else
        await assert.rejects(
          writing,
          errorCode(failure === 'unknown' ? 'OUTCOME_UNKNOWN' : 'WRITE_FAILED'),
        );
      assert.equal(f.content.writes, 2);
      assert.equal(
        s.status(b.agentHandle, queued.requestId as string).state,
        failure === 'unknown' ? 'queued' : 'offered',
      );
      if (failure === 'unknown') {
        await assert.rejects(
          s.createThread(b.agentHandle, 'Blocked', ''),
          errorCode('PROJECT_RECOVERING'),
        );
        assert.throws(() => s.forceRelease(turn.turn.id), errorCode('NOT_CLAIMED'));
        assert.equal(
          f.control.view(s => Object.values(s.pending).length),
          1,
        );
      }
    });
...
test('reconnect grace retains order; clean disconnect cancels; clock discontinuity revokes claimed turns', async t => {
  ...
  const turn = await hold(s, a.agentHandle, thread);
  const queued = await s.requestResourceTurn(b.agentHandle, thread);
  s.disconnect(b.agentHandle, false);
  s.releaseTurn(a.agentHandle, turn.turn.id, turn.turn.fencingToken);
  ...
  assert.equal(s.status(b2.agentHandle, queued.requestId as string).state, 'offered');
  s.disconnect(b2.agentHandle);
  assert.equal(s.status(c.agentHandle, third.requestId as string).state, 'offered');
  const next = (await s.claimTurn(
    c.agentHandle,
    s.status(c.agentHandle, third.requestId as string).offerId as string,
    20,
  )) as Turn;
  clock.jumped = true;
  await assert.rejects(
...
test('archive and restore serialize with appends; pagination stays within the claimed snapshot', async t => {
  ...
  assert.equal(
    ((await s.call(a.agentHandle, 'listThreads', {})) as { threads: unknown[] }).threads.length,
    1,
  );
});
