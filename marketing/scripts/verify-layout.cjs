...
(async () => {
  ...
  const cases = [
    ['banner', art.overview()],
    ['social', art.overview(1280, 640)],
    ['workflow', art.workflow()],
    ['brand', art.brandSheet()],
  ];
  for (let n = 0; n < art.DURATION * art.FPS; n++)
    cases.push([`frame ${n}`, art.frame(n / art.FPS)]);
  for (const [name, source] of cases) {
    ...
    const problems = await page.evaluate(() => {
      ...
      const elements = [...svg.querySelectorAll('text')]
        .filter(e => !e.closest('g[opacity="0.000"]'))
      ...
      for (const e of elements)
        if (
          e.r.left < -0.5 ||
          e.r.top < -0.5 ||
          e.r.right > bounds.right + 0.5 ||
          e.r.bottom > bounds.bottom + 0.5
        )
          issues.push(`Outside canvas: ${e.text}`);
  ...
  await page.screenshot({ path: root + '/qa/gallery-desktop.png', fullPage: true });
  ...
  await fs.writeFile(
    root + '/qa/layout-checks.json',
    JSON.stringify({ samples, failures, mobileWidth: 390 }, null, 2),
  );
