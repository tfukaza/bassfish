...
test('simultaneous opposite dependency edits cannot introduce a cycle', async t => {
  const f = await fixture();
  t.after(f.close);
  const ids: string[] = [];
  for (const title of ['Left', 'Right']) {
    ...
    ids.push(created.ticketId);
  }
  const handles = [f.a.agentHandle, f.b.agentHandle];
  const turns = (await Promise.all(
    handles.map((handle, index) =>
      f.service.callMcp(handle, 'acquireTurn', {
        target: { type: 'ticket', ticketId: ids[index] },
      }),
    ),
  )) as Claimed[];
  const results = await Promise.allSettled(
    turns.map((turn, index) =>
      f.service.callMcp(handles[index]!, 'commitTurn', {
        turnToken: turn.turnToken,
        mutation: { kind: 'updateTicket', dependsOn: [ids[1 - index]] },
      }),
    ),
  );
  assert.equal(results.filter(r => r.status === 'fulfilled').length, 1);
  assert.ok(results.some(r => r.status === 'rejected' && errorCode('DEPENDENCY_CYCLE')(r.reason)));
});
