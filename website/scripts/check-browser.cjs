...
(async()=>{
  ...
  try{
    ...
    for(const [width,height] of [[1440,1000],[390,844],[320,568],[1280,600]]){
      ...
      await page.locator('.feature-sections').screenshot({path:path.join(output,`${width}-benefits.png`)});
      check(`Three readable feature sections at ${width}px`,await page.locator('.feature-section').count()===3&&await page.locator('.feature-section').evaluateAll(cards=>cards.every(e=>e.clientWidth>=innerWidth/5&&e.scrollWidth<=e.clientWidth)));
    }
    ...
    for(const [name,setup] of [
      ...
      check(`${name} retains a visible text story and installation`,await fallback.locator('.story-chapters').isVisible()&&await fallback.locator('#install-code').isVisible()&&await fallback.locator('#pond canvas').count()===0&&await fallback.locator('.feature-section').count()===3&&await fallback.locator('.terminal-layer').isHidden());
      await fallback.close();
    }
    const noJS=await browser.newPage({javaScriptEnabled:false});await noJS.goto(base);
    check('No-JavaScript view includes the complete story and host instructions',await noJS.locator('.story-chapters').isVisible()&&await noJS.locator('#host-claude').isVisible()&&await noJS.locator('#host-codex').isVisible()&&await noJS.locator('.feature-section').count()===3);
