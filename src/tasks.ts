...
const baseTask = z.object({ taskId, status: z.enum(['working','input_required','completed','failed','cancelled']), statusMessage: z.string().optional(),
  createdAt: z.string().datetime(), lastUpdatedAt: z.string().datetime(), ttlMs: z.number().int().nonnegative().nullable(), pollIntervalMs: z.number().int().nonnegative().optional() }).passthrough();
export const getTaskParams = z.object({ taskId }).strict();
export const cancelTaskParams = z.object({ taskId }).strict();
export const updateTaskParams = z.object({ taskId, inputResponses: z.record(z.string(),z.unknown()) }).strict();
...
export function hasTasksCapability(envelope: unknown): boolean {
  ...
  const capabilities = (value?.clientCapabilities ?? value?.['io.modelcontextprotocol/clientCapabilities']) as { extensions?: Record<string,unknown> } | undefined;
  return Boolean(capabilities?.extensions && Object.hasOwn(capabilities.extensions,tasksExtensionId));
}
...
export function wireTask(value: Record<string,unknown>, resultType: 'task' | 'complete'): Record<string,unknown> {
