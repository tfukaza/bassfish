...
if (
  ...
  siteURL.search ||
  siteURL.hash ||
  !siteURL.pathname.endsWith('/')
) {
  throw new Error(
    'BASSFISH_SITE_URL must be an HTTP(S) URL ending in /, without a query or fragment.',
  );
}
...
await cp(path.join(site, 'theme.css'), path.join(out, 'theme.css'));
await cp(path.join(site, 'feature-fish.js'), path.join(out, 'feature-fish.js'));
...
await cp(path.join(root, 'marketing/backdrop/vendor'), path.join(out, 'backdrop/vendor'), {
  recursive: true,
});
...
await cp(
  path.join(site, 'assets/bassfish-team-social-v2.jpg'),
  path.join(out, 'assets/bassfish-team-social-v2.jpg'),
);
...
for (const name of [
  ...
  if (['habitat.js', 'bass.js', 'foliage.js'].includes(name)) {
    ...
    if ((text.match(/\.png(?=`)/g) ?? []).length !== 1)
      throw new Error(`Review texture URLs in ${name} before building.`);
    text = text.replace(/\.png(?=`)/g, '.webp');
  }
  if (name === 'index.html')
    text = text.replace('../../logo/icon-light.svg', '../../assets/icon.svg');
...
await sharp(path.join(pond, 'poster.jpg'))
  .resize({ width: 1440, withoutEnlargement: true })
  .jpeg({ quality: 85, mozjpeg: true })
  .toFile(path.join(out, 'assets/pond.jpg'));
await cp(path.join(out, 'assets/pond.jpg'), path.join(out, 'backdrop/pond/poster.jpg'));

const images = [
  'textures/soil-albedo-v2',
  'textures/grass-albedo-v2',
  'textures/sand-albedo-v1',
  'textures/rock-albedo-v1',
  ...['bank-grass', 'bank-shrub', 'bank-flowers', 'water-plants', 'cattails', 'lily-pad'].map(
    name => `foliage/${name}-v2`,
  ),
  'fish/bass-body-v2',
  'fish/bass-fins-v1',
];
...
for (const name of images) {
  ...
  const result = await sharp(original)
    ...
    .toFile(target);
  sourceBytes += (await stat(original)).size;
  outputBytes += result.size;
}
console.log(`Built ${out}`);
console.log(
  `12 texture maps: ${(sourceBytes / 1e6).toFixed(2)} MB PNG → ${(outputBytes / 1e6).toFixed(2)} MB WebP. Canonical URL: ${siteURL.href}`,
);
