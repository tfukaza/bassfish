...
try {
  ...
  function sync(){
    ...
    document.querySelector('#pause-icon').setAttribute('d',pond.paused?'M2 1l9 5-9 5Z':'M3 1v10M9 1v10');
  }
  pause.addEventListener('click',()=>{pond.setPaused(!pond.paused);sync();});
  ...
  sync();
  if(pond.reducedMotion) document.querySelector('#hint').textContent='Motion paused. Drag to look around.';
