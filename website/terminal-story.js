...
const events = [
  ...
  [0,'meta','Connected to #pagination'],
  [1,'meta','Connected to #pagination'],
  [2,'meta','Connected to #pagination'],
  [3,'meta','Connected to #pagination'],
  [1,'send','→ @api: What’s the response shape?'],
  [0,'receive','← @client: What’s the response shape?'],
  [0,'meta','Thread turn claimed'],
  [0,'send','→ @team: { items, nextCursor }. Null means the last page.'],
  [0,'meta','Message saved · turn released'],
  [1,'receive','← @api: { items, nextCursor }. Null means the last page.'],
  [2,'receive','← @api: Null means the last page.'],
  [3,'receive','← @api: Response shape agreed.'],
  [0,'meta','Note turn claimed · api-plan'],
  [0,'note','Saved the response shape in api-plan.'],
  [3,'meta','Waiting for the api-plan turn…'],
  [0,'meta','Note saved · turn released'],
  [3,'meta','Turn claimed · reading the latest note'],
  [3,'note','Added review checks to api-plan.'],
  [3,'meta','Note saved · turn released'],
  [1,'send','→ @tests: I’ll hide Next on the last page.'],
  [2,'receive','← @client: Next is hidden on the last page.'],
  ...
  [0,'send','→ @team: Endpoint is ready for testing.'],
  [2,'receive','← @api: Endpoint is ready.'],
  [1,'receive','← @api: Endpoint is ready.'],
  [1,'send','→ @tests: UI is ready too.'],
  [2,'receive','← @client: UI is ready too.'],
  [2,'task','Running pagination tests…'],
  [3,'send','→ @client: Can we label the loading state?'],
  [1,'receive','← @review: Label the loading state?'],
  [1,'task','Adding “Loading more items…”'],
  [1,'send','→ @review: Done. Ready for another look.'],
  [3,'receive','← @client: Loading state updated.'],
  [2,'note','Saved test results in qa/pagination.'],
  [2,'send','→ @team: All 12 pagination tests pass.'],
  [0,'receive','← @tests: All 12 pagination tests pass.'],
  [1,'receive','← @tests: All 12 pagination tests pass.'],
  [3,'receive','← @tests: All 12 pagination tests pass.'],
  [3,'note','Saved the review in review/pagination.'],
  [3,'send','→ @team: Reviewed. Ready to merge.'],
  [0,'receive','← @review: Ready to merge.'],
  [1,'receive','← @review: Ready to merge.'],
  [2,'receive','← @review: Ready to merge.'],
...
export function createTerminalStory(layer, pond) {
  const panes = sessions.map((session, index) => {
    ...
    element.innerHTML = '<div class="terminal-bar"><span class="terminal-host"></span><span class="terminal-agent"></span></div><div class="terminal-viewport"><div class="terminal-feed"></div></div><div class="terminal-status">bassfish <span>•</span> #pagination</div>';
  ...
  let lastLayout = '', lastEvent = -1;

  return frame => {
    const { morph, width, height, progress, pondRects } = frame;
    layer.hidden = morph === 0;
    pond.style.opacity = String(1 - ease(.08, .78, morph));
    if (!morph) return;
    const gap = width < 760 ? 10 : 18;
    const gridWidth = Math.min(1080, width - (width < 760 ? 40 : 80));
    const gridHeight = Math.min(580, height * .65);
    const paneWidth = (gridWidth - gap) / 2, paneHeight = (gridHeight - gap) / 2;
    const left = (width - gridWidth) / 2, top = height * .56 - gridHeight / 2;
    ...
    layer.style.opacity = String(ease(0, .42, morph));
    for (const [index, pane] of panes.entries()) {
      ...
      pane.element.style.setProperty('--terminal-ink', String(ease(.24, .88, morph)));
    }
    ...
    lastLayout = layout; lastEvent = shown;
  };
}
